...
((delay)=>{
	...
	//F3 is function to draw a sigle triangular plane on the canvas tag
	F3=()=>{
		let slf=window.document,C=slf.getElementById('ndrandomCvs3'),ctx=C.getContext('2d'),
			Rs=Math.sampleWeb2_random(Math.random()),canvasW=C.width,canvasH=C.height,
			x0=canvasW*0.5,y0=canvasH*0.5,X1=0.0,X2=0.0,X3=0.0,Y1=0.0,Y2=0.0,Y3=0.0;
		//
		//Rs: [x1 dx1 x2 dx2 ... x11 dx11] where Xi = xi+c*dxi and c is a constant
		//Rs[0] -> x1: relative x coordinate of an origin (= width/2)
		//Rs[2] -> x2: relative y coordinate of an origin (= height/2)
		//
		//Rs[4] -> x3: the first relative distance from the origin
		//Rs[6] -> x4: the second relative distance from the origin
		//Rs[8] -> x5: the third relative distance from the origin
